...
# A road stretch on a state border is generated by BOTH neighbouring states'
# pipelines: same segment name, centroids ~metres apart. Each such stretch
# belongs in the database once, under the state holding most of it (163 were
# de-duplicated on that rule on 2026-09-20).
BORDER_TOLERANCE_DEG = 0.0002  # ~22 m


def is_border_duplicate(name: str, lat: float, lng: float, other_states_zones: dict[str, list[tuple[float, float]]]) -> bool:
    """True if a zone with this name already exists in ANOTHER state at
    (almost) this exact spot -- i.e. the neighbouring state already owns this
    stretch, so loading it again would count the same road twice."""
    return any(
        abs(lat - other_lat) < BORDER_TOLERANCE_DEG and abs(lng - other_lng) < BORDER_TOLERANCE_DEG
        for other_lat, other_lng in other_states_zones.get(name, ())
    )
...
def create_or_get_zones(features: list[dict], state: str = "Sikkim", limit: int | None = None,
    ...
    try:
        ...
        zones = db.query(Zone.name, Zone.state, Zone.id, Zone.centroid_lat, Zone.centroid_lng).all()
        existing = {(name, st): str(zid) for name, st, zid, _, _ in zones}
        elsewhere: dict[str, list[tuple[float, float]]] = {}
        for name, st, _, lat, lng in zones:
            if st != state:
                elsewhere.setdefault(name, []).append((lat, lng))
        selected = features[:limit] if limit else features
        skipped_border: set[str] = set()  # names of stretches the neighbouring state already owns
        ...
        for feat in selected:
            ...
            if (name, state) not in existing:
                ...
                if is_border_duplicate(name, centroid.y, centroid.x, elsewhere):
                    skipped_border.add(name)
                    continue
        ...
        if skipped_border:
            print(f"  skipped {len(skipped_border)} border stretch(es) already owned by a neighbouring state")
        for feat in selected:
            props = feat["properties"]
            name = zone_name_for(props)
            if name in skipped_border:
                continue
