...
- **Two rain rules; alerting is per state.** Sikkim uses its own published landslide
  study. Every other state uses a national baseline, `IMD_HEAVY_RAIN` in `app/config.py`:
  the India Meteorological Department's "heavy rain" category, 64.5 mm or more in 24 hours
  (the susceptibility rule lowers it 20% for high-risk roads: 51.6 mm). It is a rainfall
  category IMD uses for its own warnings, **not a landslide-specific threshold**, and was
  confirmed from published summaries rather than IMD's own page. Which states may actually
  alert is an explicit list, `RAINFALL_ALERT_STATES` (default Sikkim only; the others
  refresh rainfall and show the static ML layer). Nothing is ever borrowed from another
  state's landslide study.
  **Why Assam does not use its own Guwahati equation** (Bhusan et al. 2014, coefficient
  30.9 after converting mm/h and hours to mm/day and days): it was fit to 19 landslides in
  one city, and on the 22 days of rainfall stored on 2026-09-21 all 25 of Assam's
  highest-risk zones were over it on most days, even a day with ~0 mm, because its
  15-20 day windows sit at ~6 mm/day. That would be a permanent false alarm with real SMS.
  The same check with the IMD line fired once, on one zone. To switch Assam on, delete its
  `RAINFALL_THRESHOLDS__ASSAM__*` variables on Render (so the baseline applies) and set
  `RAINFALL_ALERT_STATES=["sikkim","assam"]`; the code default stays Sikkim so nothing
  alerts by surprise. Only the same still-unverified caveat as any other state applies:
  the daily totals are Open-Meteo model rainfall, not IMD gauges.
...
state on the alerting list** (`RAINFALL_ALERT_STATES`, default Sikkim only). A state
not on the list is refreshed but does not alert (see above). It reads
...
**Downloading records.** The Incidents page has a "Download CSV" button that exports every
record matching the state and filters on screen (`GET /landslide-records/export.csv`; UTF-8
with a byte-order mark so Excel opens it cleanly; text starting with = + - @ is prefixed
with an apostrophe so a spreadsheet cannot run it as a formula).

**Alerts for Assam:** the Guwahati equation would flag ordinary monsoon rain statewide (see
"Two rain rules" above), so Assam uses the IMD baseline once it is put on the alerting list
-- a step that is configuration on Render, not code.
