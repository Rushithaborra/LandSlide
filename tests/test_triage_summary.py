"""Unit tests for app/services/triage_summary.py -- the Claude API client is
...
def test_generate_triage_summary_returns_none_on_api_error():
    with patch.object(triage_summary, "get_client", side_effect=RuntimeError("Anthropic API key not configured")):
...
def test_anthropic_configured_false_without_key():
    with patch.object(triage_summary.settings, "anthropic_api_key", None):
        assert triage_summary.anthropic_configured() is False


def test_anthropic_configured_true_with_key():
    with patch.object(triage_summary.settings, "anthropic_api_key", "sk-ant-test"):
        assert triage_summary.anthropic_configured() is True
